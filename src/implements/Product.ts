...
class Product {
  ...
  async FGProductAddExt(req: Request, res: Response) {
    try {
      let rg = new resGen();
      await client.connect();
  ...
  async FGProductAddBDExt(req: Request): Promise<any> {
    const rg = new resGen();
    try {
      //TODO implementar consulta a la base de datos para insertar producto
      // const result = await client
      ...
      //     }
      //   );
        const result = await client.db("SAProject").collection("Categoria").insertOne(req.body);
      rg.valid = true;
      rg.data = result;
    } catch (error) {
      rg.valid = false;
  ...
  async FGCatalogueExt(req: Request, res: Response) {
    try {
      let rg = new resGen();
      await client.connect();
